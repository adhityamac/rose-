# rose_v9_macstyle.py
"""
Rose v9 — MacOS style HUD, top-right buttons, typing animation,
Edge TTS for human voice, YouTube playback, always-listening voice input.
"""
...
from PySide6.QtWidgets import (
    QApplication, QWidget, QTextEdit, QVBoxLayout, QHBoxLayout, QPushButton, QGraphicsDropShadowEffect,
...
# ---------------- Typing animation ----------------
def typing_effect(widget, text, delay=0.02):
    for c in text:
        widget.insertPlainText(c)
        widget.repaint()
        time.sleep(delay)
...
# ---------------- Command processing ----------------
def process_command(command, text_widget):
    ...
    typing_effect(text_widget, f"Rose: {resp}")
    talk(resp)
...
class RoseHUD(QWidget):
    ...
    def init_ui(self):
        ...
        self.append_text("Rose v9 online — always listening...")
    ...
    def append_text(self, t):
        typing_effect(self.text_area, t)

    def on_command(self, command):
        typing_effect(self.text_area, f"You said: {command}")
        threading.Thread(target=process_command, args=(command, self.text_area), daemon=True).start()
