# rose_v20_polished_neonhud.py
import sys, asyncio, threading, webbrowser, platform, subprocess
...
def handle_command(cmd: str, hud_ref):
    ...
    if "youtube" in cmd or "play" in cmd:
        ...
        hud_ref.update_response("Hello! How can I help you today?")
    else:
...
class NeonHUD(QWidget):
    def __init__(self):
        ...
        # Greeting
        QTimer.singleShot(1000, lambda: self.greet_user())

    def close_app(self):
        global vlc_process
        if vlc_process:
            vlc_process.kill()  # Close VLC/TTS process if running
        self.close()

    def greet_user(self):
        greet_text = "Hello! I am Rose. How can I help you today?"
        self.update_response(greet_text)
        speak(greet_text)
